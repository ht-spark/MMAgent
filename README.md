...
面向数学建模竞赛的智能体。MMAgent 将题目文档与附件数据组织为一个可复现的解题过程：先理解问题和数据，再按小问递进完成数学模型构建、计算、验证与结果沉淀，最后生成论文草稿、进行论文审查并交付所有材料。它的目标不是机械地选择复杂模型，而是在竞赛约束下形成**题意匹配、数据可信、计算可复现、结论可解释**的建模方案。
