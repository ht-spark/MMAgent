...
> 从读题、数据画像、方法探索、建模计算、结果验证，到论文撰写与全题审查的全流程自动化智能体系统。

MMAgent 将题目文档与附件数据组织为一个可复现的解题过程：先理解问题与数据，再按小问递进完成模型构建、计算、验证与结果沉淀，最后生成论文草稿、进行审查并交付全部材料。
...
**数据流**：前端提交题目与附件 → 服务层落库并起后台任务 → 核心引擎跑 LangGraph 图，节点进度通过回调写回 SQLite → 前端轮询 `/api/runs/{id}` 获取进度与产物清单 → 完成后下载论文、图表与代码。
...
- **题目理解与小问拆分**：提取研究对象、背景机制、显式与隐式约束，自动拆分小问并建立依赖关系图与题目-数据映射表。
...
- **论文写作**：从已验证的小问结果包生成完整竞赛论文草稿（Markdown），包含摘要、问题重述、模型假设、符号说明、各小问模型建立与求解、模型评价与参考文献，并自动转换为 DOCX。
- **全题审查与交付**：完成所有小问后进行一致性审查（逻辑连贯、数据口径一致、数值可追溯），通过格式与内容审查后交付最终产物包。
...
读取题目文档与全部附件，生成数据画像，提取小问列表与依赖关系。`G0` 质量门检查：小问是否完整提取、附件是否全部读取、依赖关系是否无环、关键数据缺口是否已记录。
...
所有小问完成后，进行全题一致性审查，然后生成论文草稿，经格式与内容审查通过后交付。`GF` 门未通过时自动修订重写，通过后交付最终产物包。
...
方法候选完全由联网搜索与大模型思考生成，不依赖预设方法目录或注册表，能够根据题目特点灵活匹配最合适的方法。无网络时降级为按任务类型的通用候选，确保工作流不中断。
...
对联网检索次数、方法候选数量、代码修复次数、验证迭代次数、时间与令牌设有预算。预算紧张时按优先级降级：减少低价值候选 → 优先简单基线 → 减少非关键图表 → 保留必要验证，但不得跳过数据质量检查、数值复现与题目覆盖检查。
...
- **结果自评**：求解后 LLM 自评是否真正回答题目、数值是否合理，`revise` 时携带建议自动重算一次（反思循环）。
- **论文写作**：模型建立与结果解释核心段落由 LLM 起草（禁止编造数字），失败回退确定性模板。
...
题目参数可直接传入文本或文件路径：`--problem examples/problem.md` 或 `--problem "某工厂生产A、B两种产品..."`。
...
| 方法       | 路径                                | 说明                                                                  |
| ---------- | ----------------------------------- | --------------------------------------------------------------------- |
| `POST`   | `/api/runs`                       | 提交解题任务（表单：题目文本/文件 + 附件 + llm_config），后台异步执行 |
| `GET`    | `/api/runs`                       | 运行历史列表                                                          |
...
| `GET`    | `/api/runs/{run_id}/paper`        | 获取论文 Markdown 文本                                                |
...
> `questions/<qid>/` 仅在"题目驱动建模"路径（配置了 LLM 且题型为 optimization / stochastic_optimization / evaluation / prediction / simulation）生成；无 LLM 或回退到预设方法时，该小问不产生代码文件（计算由确定性内置函数完成）。
...
- **tenacity** — API 调用重试与容错；**python-docx** — 论文 Markdown 到 DOCX 转换；**python-dotenv** — 环境变量管理。
